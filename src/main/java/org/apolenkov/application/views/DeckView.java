...
public class DeckView extends Composite<VerticalLayout>
  ...
  public DeckView(
      DeckUseCase deckUseCase, FlashcardUseCase flashcardUseCase, DeckPresenter presenter) {
  ...
  private void createActions() {
    ...
    Button practiceButton =
        new Button(getTranslation("deck.startSession"), VaadinIcon.PLAY.create());
    practiceButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY, ButtonVariant.LUMO_SUCCESS);
    practiceButton.addClickListener(
        e -> {
          if (currentDeck != null) {
            getUI()
                .ifPresent(ui -> ui.navigate(PracticeView.class, currentDeck.getId().toString()));
          }
        });

    Button addFlashcardButton =
        new Button(getTranslation("deck.addCard"), VaadinIcon.PLUS.create());
    ...
    editDeckButton.addClickListener(
        e -> {
          if (currentDeck != null) {
  ...
  private void createFlashcardsGrid() {
    ...
    Button resetProgress =
        new Button(getTranslation("deck.resetProgress"), VaadinIcon.ROTATE_LEFT.create());
    resetProgress.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_ERROR);
    resetProgress.addClickListener(
        e -> {
          if (currentDeck != null) {
            presenter.resetProgress(currentDeck.getId());
            Notification.show(
                getTranslation("deck.progressReset"), 2000, Notification.Position.BOTTOM_START);
            loadFlashcards();
          }
        });
    ...
    flashcardGrid
        .addComponentColumn(
            flashcard -> {
              boolean known =
                  currentDeck != null && presenter.isKnown(currentDeck.getId(), flashcard.getId());
    ...
    flashcardGrid
        .addComponentColumn(
            flashcard -> {
              ...
              toggleKnown
                  .getElement()
                  .setProperty("title", getTranslation("deck.toggleKnown.tooltip"));
              toggleKnown.addClickListener(
                  e -> {
  ...
  private void applyFlashcardsFilter() {
    ...
    String q =
        flashcardSearchField != null && flashcardSearchField.getValue() != null
            ? flashcardSearchField.getValue().toLowerCase().trim()
            : "";
    Set<Long> knownIds = presenter.getKnown(currentDeck.getId());
    boolean hideKnown =
        hideKnownCheckbox != null && Boolean.TRUE.equals(hideKnownCheckbox.getValue());
    flashcardsDataProvider.clearFilters();
    flashcardsDataProvider.addFilter(
        fc -> presenter.filterFlashcards(List.of(fc), q, knownIds, hideKnown).contains(fc));
  }
    ...
    saveButton.addClickListener(
        e -> {
          try {
            ...
            Notification.show(
                flashcard == null
                    ? getTranslation("deck.card.added")
                    : getTranslation("deck.card.updated"),
                2000,
  ...
  private void deleteFlashcard(Flashcard flashcard) {
    ...
    confirmButton.addClickListener(
        e -> {
          ...
          confirmDialog.close();
          Notification.show(
              getTranslation("deck.card.deleted"), 2000, Notification.Position.BOTTOM_START);
        });
