...
public class HomePresenter {
                  ...
                  d ->
                      (d.getTitle() != null
                          ...
                          || (d.getDescription() != null
                              && d.getDescription().toLowerCase(Locale.ROOT).contains(normalized)))
              .collect(Collectors.toList());
    }

    return decks.stream()
        .sorted(
            Comparator.comparing(Deck::getTitle, Comparator.nullsLast(String::compareToIgnoreCase)))
  ...
  public DeckCardViewModel toViewModel(Deck deck) {
    ...
    return new DeckCardViewModel(
        deck.getId(), deck.getTitle(), deck.getDescription(), deckSize, known, percent);
  }
}
